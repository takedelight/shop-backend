...
describe('UserService', () => {
  ...
  it('should handle inactive user', async () => {
    ...
    // Act & Assert
    await expect(target.findById('user-123')).rejects.toThrow(
      'User is inactive',
    );
  });
});
...
it('should process paid order', async () => {
  ...
  const order = orderBuilder()
    .withItems([{ productId: 'prod-1', quantity: 2, price: 100 }])
    .paid()
    .build();
...
export function createUserDtoFixture(
...
export function createUpdateUserDtoFixture(
...
export function createPaginationQueryFixture(
...
export function createPaymentResponseFixture(
