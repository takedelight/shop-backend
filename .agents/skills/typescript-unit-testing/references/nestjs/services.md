...
describe('update', () => {
  ...
  it('should throw NotFoundException when user not exists', async () => {
    ...
    // Act & Assert
    await expect(target.update('invalid', { name: 'New' })).rejects.toThrow(
      NotFoundException,
    );
  });
});
...
describe('sendNotification', () => {
  ...
  it('should handle email service failure gracefully', async () => {
    ...
    // Act & Assert
    await expect(
      target.sendNotification('user-123', 'Welcome!'),
...
describe('transferFunds', () => {
  it('should rollback on failure', async () => {
    ...
    mockRepository.updateBalance.mockResolvedValueOnce({
      ...fromAccount,
      balance: 50,
    });
    ...
    // Act & Assert
    await expect(target.transferFunds('acc-1', 'acc-2', 50)).rejects.toThrow(
      InternalException,
    );
