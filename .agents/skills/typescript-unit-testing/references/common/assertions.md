...
it('should throw NotFoundException when user not found', async () => {
  ...
  // Act & Assert
  await expect(target.getUser('non-existent-id')).rejects.toThrow(
    NotFoundException,
  );
});
...
it('should throw ValidateException for invalid input', async () => {
  ...
  // Act & Assert
  await expect(target.createUser(invalidInput)).rejects.toThrow(
    ValidateException,
  );
...
it('should propagate InternalException from repository failure', async () => {
  ...
  // Act & Assert
  await expect(
    target.createUser({ email: 'test@example.com', name: 'John' }),
