...
describe('UserService', () => {
  ...
  describe('findById', () => {
    ...
    it('should throw NotFoundException when user not found', async () => {
      ...
      // Act & Assert
      await expect(target.findById('invalid-id')).rejects.toThrow(
        NotFoundException,
      );
      await expect(target.findById('invalid-id')).rejects.toMatchObject({
...
describe('UserService - Error Cases', () => {
  describe('findById', () => {
    it('should throw NotFoundException when user not found', async () => {
      ...
      // Act & Assert
      await expect(target.findById('non-existent')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('create', () => {
    it('should throw ValidateException when email already exists', async () => {
      ...
      // Act & Assert
      await expect(
        target.create({ email: 'existing@example.com', name: 'John' }),
  ...
  describe('delete', () => {
    it('should throw ForbiddenException when deleting admin user', async () => {
      ...
      // Act & Assert
      await expect(target.delete('admin-1')).rejects.toThrow(
        ForbiddenException,
      );
    });
  });
  ...
  describe('external service failures', () => {
    it('should throw InternalException when email service fails', async () => {
      ...
      // Act & Assert
      await expect(
        target.create({ email: 'test@example.com', name: 'John' }),
...
describe('Exception Behavior', () => {
  it('should throw NotFoundException with correct error code', async () => {
    ...
    // Act & Assert
    await expect(target.findById('invalid')).rejects.toMatchObject({
      errorCode: ErrorCode.USER_NOT_FOUND,
...
it('should resolve to user', async () => {
  ...
  // Act & Assert
  await expect(target.findById('user-123')).resolves.toEqual({
    id: 'user-123',
  });
});
