...
describe('emit', () => {
  ...
  it('should handle emit error gracefully', async () => {
    ...
    // Act & Assert
    await expect(
      target.emitUserCreated({ userId: '123' }),
...
describe('send', () => {
  ...
  it('should handle timeout on send', async () => {
    ...
    // Act & Assert
    await expect(target.getUserProfileWithTimeout('user-123')).rejects.toThrow(
      'Request timeout',
    );
  });
});
...
describe('UserMessageHandler', () => {
  ...
  describe('handleGetUser (@MessagePattern)', () => {
    ...
    it('should handle service error', async () => {
      ...
      // Act & Assert
      await expect(
        target.handleGetUser({ userId: 'user-123' }),
...
describe('EventHandler', () => {
  ...
  describe('handleOrderCreated (@EventPattern)', () => {
    it('should process order created event', async () => {
      ...
      ).toHaveBeenCalledWith(event.userId, event.orderId);
    });
    ...
    it('should not throw on handler error (fire-and-forget)', async () => {
      ...
      // Act & Assert
      await expect(target.handleOrderCreated(event)).resolves.not.toThrow();
    });
  });
});
```
...
describe('Error Handling', () => {
  ...
  it('should send to dead letter queue on permanent failure', async () => {
    ...
    // Assert
    expect(mockDeadLetterService.send).toHaveBeenCalledWith('orders.dlq', {
      payload,
      error: error.message,
    });
  });
